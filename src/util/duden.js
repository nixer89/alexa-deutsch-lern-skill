...
const
	MAX_CONNECTIONS = 3,
	DUDEN_SEARCH_URL = "https://www.duden.de/suchen/dudenonline/",
	...
	parseWordDefinition ( word, result ) {
		try {
			...
			let $dom = $( "body" ),
				...
				$grammar = $dom.find( "h2:contains(Grammatik)" ).closest( ".block" ).find( ".lexem" ),
				$other = $dom.find( "h2:contains(Grammatik)" ).closest( ".block" ).find( ".wide" ),
